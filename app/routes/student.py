...
def signup():
    ...
    if form.validate_on_submit():
        ...
        # Create profile (initially without reg_no)
        profile = Profile(
            user_id=user.id,
        ...
        db.session.commit()
        # Assign reg_no after profile.id is available
        class_code_map = {
            '6': '6', '7': '7', '8': '8', '9': '9', '10': '10',
            '11_arts': '11A', '11_science': '11S',
            '12_arts': '12A', '12_science': '12S'
        }
        class_code = class_code_map.get(form.student_class.data, 'X')
...
def upi_payment(fee_id):
    ...
    phone_no = phone_setting.value if phone_setting else None
    if request.method == 'POST':
...
@student_bp.route('/qr')
def qr():
    qr_url = url_for('static', filename='img/upi_qr.jpg')
